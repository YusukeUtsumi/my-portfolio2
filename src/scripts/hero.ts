const root = document.querySelector<HTMLElement>('.hero-wrap');
const orbsContainer = document.querySelector<HTMLElement>('.orbs');
...
if (root && orbsContainer) {
    ...
    const ensureTinyOrbs = () => {
        ...
        if (isDesktop()) {
            ...
            if (mobileCount > 0) makeTinyOrbs(mobileCount);
        }
    };
    ...
    const loop = () => {
        ...
        frame++;
        if (frame % 2 === 0) if (!raf) raf = requestAnimationFrame(updateVars);
